<template>
  <div class="container">
    ...
    <div style="height: 500px" ref="gantt" class="gantt-container"></div>
  </div>
</template>
<script>
import {
...
export default {
  ...
  methods: {
    ...
    initData: function() {
      this.tasks.data1 =[
        { id: 3, toolTipsTxt: 'xxx项目-项目启动会',status: "yellow",text:"Team", type:'milestone', start_date: new Date('2022-04-27')},
        { id: 1, text: '1222',status: "pink",start_date:'25-04-2022', end_date:'01-07-2022', open:true },
      ...
      this.tasks.data = [{
        ...
        var newObj = {}
        if (current.colortype) { //存在type字段 说明非一级菜单，判断阶段的具体类型 设置不同颜色
          if (current.colortype == 1) { //冒烟
            newObj = Object.assign({}, current, {
              'color': '#fcca02'
            })
          } else if (current.colortype == 2) { //单元
            newObj = Object.assign({}, current, {
              'color': '#fec0dc'
            })
          } else if (current.colortype == 3) { //回归
            newObj = Object.assign({}, current, {
              'color': '#62ddd4'
            })
          } else if (current.colortype == 4) {
            newObj = Object.assign({}, current, {
              'color': '#d1a6ff'
            })
          }
